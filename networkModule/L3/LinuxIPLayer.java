...
import config.configFiles.IpForwardFile;
...
//    public boolean ip_forward = true;
    ...
    public LinuxIPLayer(IpNetworkModule netMod) {
        super(netMod);
        this.ttl = 64;
        this.forwardFile = new IpForwardFile(netMod.getDevice().getFilesystem());
    }
    ...
    /**
     * Handles packet: is it for me?, routing, decrementing TTL, postrouting,
     * MAC address finding.
     *
     * @param packet
    ...
    protected void handleReceiveIpPacket(IpPacket packet, EthernetInterface iface) {
        String name = netMod.getDevice().getName();
