...
public class SaveLoadManagerNetworkModel extends AbstractSaveLoadManager {
     ...
     * @return
     */
    public SaveLoadManagerUserReactionSimulation doAskUserIfSaveBeforeSimulation(Graph graph) {
        //save config data
        int i = showWarningNotSavedBeforeSimulation(dataLayer.getString("WINDOW_TITLE"), dataLayer.getString("SIMULATING_NOT_SAVED_PROJECT"));

        // if canceled
        if (i == 2 || i == -1) {
            // do nothing
            //return false;
            return SaveLoadManagerUserReactionSimulation.CANCEL;
        }

        // if YES -> save
        if (i == 0) {
            //boolean result = doSaveGraphAction();
            return SaveLoadManagerUserReactionSimulation.DO_SAVE;
        }
