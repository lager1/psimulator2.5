...
// Privatni metody resici sitovou komunikaci: ------------------------------------------------------------------------
	...
	private void handleReceivePacket(L2Packet packet, int switchportNumber) {
		if (packet.getClass() != EthernetPacket.class) {	//kontrola spravnosti paketu
			ladiciVypisovani("Zahazuju paket, protoze neni tridy " + packet.getClass().getName());
			System.exit(2);
		} else {
			...
			if(swport == null){
				ladiciVypisovani("Prisel paket na switchport, o jehoz existenci nemam tuseni: switchport c.: " + switchportNumber);
				System.exit(2);
			}
			//kontrola, bylo-li nalezeno rozhrani
			EthernetInterface iface = swport.assignedInterface;
			if (iface == null) {
				ladiciVypisovani("Nenalezeno interface ke switchportu, prusvih!");
				System.exit(2);
			}
...
// Konstruktory a nastavovani pri startu: -------------------------------------------------------------------------------------------------
	...
	/**
	 * Explores hardware.
	 * Pri startu projde vsechny switchporty fysickyho modulu a nacte si je.
	 */
	...
	/**
	 * Z konfigurace priradi interfacy ke switchportum.
	 * TODO: implementovat
	 */
